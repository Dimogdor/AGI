...
function drawFilmGrade(){
  ...
  sh.addColorStop(0,'rgba(255,255,255,1)'); sh.addColorStop(1,'rgba(214,228,232,1)');   // ombres froides en bas
  ctx.fillStyle=sh; ctx.fillRect(0,0,W,H);
  ctx.globalCompositeOperation='lighter';
  const hl=ctx.createLinearGradient(0,0,0,H*0.5);
  hl.addColorStop(0,'rgba(30,18,6,0.5)'); hl.addColorStop(1,'rgba(0,0,0,0)');            // hautes lumières dorées
  ctx.fillStyle=hl; ctx.fillRect(0,0,W,H*0.5);
  ctx.globalCompositeOperation='overlay'; ctx.globalAlpha=0.05;
  const g=grainCanvas(), ox=(Math.random()*160)|0, oy=(Math.random()*160)|0;
  for (let y=-oy; y<H; y+=160) for (let x=-ox; x<W; x+=160) ctx.drawImage(g, x, y);
...
function drawAmbient(t, dev, vw){
  ...
  if (!AMB.ev && t>AMB.next){
    ...
    if (roll < 0.30+dev*0.55){
      const kinds = dev>0.3? ['plane','heli','carfire','plane'] : ['plane','heli','carfire'];
      const k = kinds[(Math.random()*kinds.length)|0];
      const dir = Math.random()<0.5? 1:-1;
      AMB.ev = { k, t:0, dir, dur: k==='plane'? 7.5 : k==='heli'? 9 : 0.8,
                 y: 60+Math.random()*70, cx: WORLD*(0.2+Math.random()*0.6) };
    }
    AMB.next = t + lerp(34, 12, dev) + Math.random()*22;
  }
  if (AMB.ev){
    const e=AMB.ev; e.t += dt;
    if (e.t>=e.dur){
      // CRASH : flash, débris, et une colonne de fumée persistante à l'horizon
      if (e.k!=='carfire'){
        if (AMB.smokes.length<4) AMB.smokes.push({cx:e.cx, t:0, life:50+Math.random()*25});
        addLight(e.cx, GROUND-140, '#ff9a4a', 130, 0.6);
      } else if (AMB.fires.length<3) AMB.fires.push({cx:e.cx, t:0, life:26});
      AMB.ev=null;
...
function drawForeground(dev, t, vw){
  if (!qHi()) return;
  ctx.save();
  if (CANF) ctx.filter='blur(1.3px)';
  ctx.fillStyle='rgba(14,10,9,0.85)'; ctx.strokeStyle='rgba(14,10,9,0.85)';
  const par=1.22;
  for (let wx=Math.floor(camX*par/230)*230; wx<camX*par+vw+230; wx+=230){
    ...
    if (h1<0.28){                                                        // touffe d'herbes hautes
      ctx.lineWidth=2.4; ctx.lineCap='round';
      for (let b2=0;b2<5;b2++){ const bx=x+b2*4-8, sw=Math.sin(t*1.3+b2+wx)*2.4;
        ctx.beginPath(); ctx.moveTo(bx,base); ctx.quadraticCurveTo(bx+sw,base-13,bx+sw*1.7,base-21-h1*14); ctx.stroke(); }
    } else {                                                             // gravats / pierre
      ctx.beginPath(); ctx.moveTo(x-9,base); ctx.lineTo(x-2,base-8-h1*6); ctx.lineTo(x+7,base-3); ctx.lineTo(x+10,base); ctx.closePath(); ctx.fill();
    }
  }
  ctx.restore();
}
...
function drawBG(){
  ...
  // PROFONDEUR DE CHAMP : les plans les plus lointains (montagnes, ville) sont réellement
  // FLOUS — l'œil fait immédiatement la mise au point sur le champ de bataille.
  const dof = CANF && qHi();
  if (dof){ ctx.save(); ctx.filter='blur(1.8px)'; }
  ...
  if (dof){ ctx.save(); ctx.filter='blur(1.2px)'; }
  drawCity(dev);
  if (dof) ctx.restore();
  drawAmbient(t, dev, vw);       // monde vivant : crashs d'avions/hélicos, incendies, fumées
  ...
  if (qFx() && game){
    ...
    const step = SETTINGS.quality==='ultra'?22 : SETTINGS.quality==='high'?30 : 46;
    for (let sx=10; sx<=vw; sx+=step){
      ...
      const hgt = 11*(1-Math.min(0.75,crush));                 // le brin s'ÉCRASE sous le pas
